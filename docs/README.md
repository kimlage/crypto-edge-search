...
We tested **31 distinct crypto trading hypotheses** under a deliberately harsh
anti-overfitting validation harness. **29 were killed.** The **2 survivors** are
...
killed 29 in-sample Sharpes that would otherwise have looked like wins.
...
| [`RESULTS.md`](RESULTS.md) | **The findings, as tables** — the per-hypothesis result tables and the headline tally (31 tested / 29 killed / 2 sub-RF survivors), with the binding gate that killed each. |
...
| [`VALIDATION_HARNESS.md`](VALIDATION_HARNESS.md) | **The reusable harness** — `validateStrategy(...)`, the single API that packages the gates so any future hypothesis is validated exactly the way these 31 were. |
...
1. **This index** — get the honest headline (31 / 29 / 2, sub-RF survivors, methodology
...
4. **[`RESULTS.md`](RESULTS.md)** — the per-hypothesis tables, to see the gates applied 31

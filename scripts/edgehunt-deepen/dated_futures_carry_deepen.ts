...
function simulateContract(
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    // excess-of-RF benchmark: the carry must beat T-bills on its CAPITAL (1 unit), not its leverage.
    // The capital tied up is the margin/cash = 1 unit; RF opportunity cost is on that 1 unit.
    if (cfg.excessOfRF) {
      dayRet -= rfDaily(cur.date);
    }
...
function main() {
  ...
  fs.writeFileSync(path.join(OUT, "dated_futures_carry_deepen_report.json"), JSON.stringify(report, null, 2));
